...
class ApiRepositorieLoginImplement implements ApiRepositoryLoginInterface {
  ...
  Future login(String username, String password, VoidCallback? Function(int code, dynamic data) callback) async {
    try {
      ...
      print("url >>> ${"${ApiGlobalUrl.generalLink}${ApiGlobalUrl.getLogin}$username/$password"}");
  ...
  @override
  Future getDataDeviceId(int id, VoidCallback? Function(int code, dynamic data) callback) async {
    try {
      final response = await dio.get("${ApiGlobalUrl.generalLink}${ApiGlobalUrl.getDataDeviceId}$id");
      if(kDebugMode){
