...
class CustomChartStd extends StatelessWidget {
  const CustomChartStd({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    final List<SalesData> chartData = pvPrincipal.modelosNodosGraficos!.lineData!
        .where((item) => item.nombre == "std") // Filtrar por nombre
        .expand((item) =>
        List.generate(item.x!.length,
                (index) =>
                SalesData(DateTime.parse('${item.x![index]}'), item.y![index])))
        .toList();
    ...
    return chartData.isNotEmpty
      ...
      primaryXAxis: DateTimeAxis(
        ...
        title: AxisTitle(
          text: 'Fecha y Hora',
          textStyle: const TextStyle(
            color: Colors.black, // Título del eje X
            fontSize: 12,
          ),
        ),
